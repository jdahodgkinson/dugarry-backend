...
var getQualiTable = function () {
    var buf = fs.readFileSync('./public/qualiRank.json');
...
var standingSort = function (first, second) {
    var pointsDiff = first.points - second.points;
    if (pointsDiff != 0) {
        return pointsDiff;
    }
    var goalDiff = first.goal_diff - second.goal_diff;
    if (goalDiff != 0) {
        return goalDiff;
    }
    var scoredDiff = goalsScored(first) - goalsScored(second);
    if (scoredDiff != 0) {
        return scoredDiff;
    }
    var winsDiff = first.wins - second.wins;
    if (winsDiff != 0) {
        return winsDiff;
    }
    var fairDiff = first.fairplay_points_lose - second.fairplay_points_lose;
    if (fairDiff != 0) {
        return fairDiff;
    }
    var qualiTable = getQualiTable();
    var fstCode = first.short_code;
    var sndCode = second.short_code;
    var qualiDiff = qualiTable[fstCode] - qualiTable[sndCode];
    return qualiDiff;
};
